...
SDL_Rect argss_rect_getsdl(VALUE self) {
	SDL_Rect rect;
	rect.x = NUM2INT(rb_iv_get(self, "@x"));
	rect.y = NUM2INT(rb_iv_get(self, "@y"));
	rect.w = NUM2INT(rb_iv_get(self, "@width"));
	rect.h = NUM2INT(rb_iv_get(self, "@height"));
	return rect;
}
